...
class GeometryValue 
  ...
  # The following are added methods to the GeometryValue class.
  # 1st added method for eval_prog when just returns self.
  def eval_prog env
    self
  end

  # 2nd added method for preprocess_prog when just returns self.
  def preprocess_prog
    self
  end
end
...
class Point < GeometryValue
  ...
  # Problem #4
  def intersectWithSegmentAsLineResult seg
    # This local method is used to check if the point is within the segment bounds.
...
class LineSegment < GeometryValue
  ...
  # Problem #4
  # The same algorithm from the hw7.sml.
  def intersectWithSegmentAsLineResult seg
    if real_close(x1, x2)
      if y2 < seg.y2
        a = self
